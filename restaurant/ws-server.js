...
// The PC acts as the single source of truth.
// All state is held in memory and persisted to a JSON file.
// Clients send mutations; server updates state and broadcasts to all.
...
// ── Seed data (same structure as client seed) ───────────────
function seedData() {
    return {
        settings: {
        ...
        modifierGroups: [
            { id: 'mg1', name: 'Size', required: true, multiSelect: false, options: [{ id: 'mg1-s', name: 'Small', price: 0 }, { id: 'mg1-m', name: 'Medium', price: 200 }, { id: 'mg1-l', name: 'Large', price: 400 }] },
            { id: 'mg2', name: 'Spice Level', required: false, multiSelect: false, options: [{ id: 'mg2-mild', name: 'Mild', price: 0 }, { id: 'mg2-medium', name: 'Medium', price: 0 }, { id: 'mg2-hot', name: 'Hot', price: 0 }, { id: 'mg2-extra', name: 'Extra Hot', price: 0 }] },
            { id: 'mg3', name: 'Add-ons', required: false, multiSelect: true, options: [{ id: 'mg3-cheese', name: 'Extra Cheese', price: 150 }, { id: 'mg3-bacon', name: 'Bacon', price: 200 }, { id: 'mg3-avocado', name: 'Avocado', price: 250 }, { id: 'mg3-egg', name: 'Fried Egg', price: 150 }, { id: 'mg3-mushroom', name: 'Mushrooms', price: 100 }] },
            { id: 'mg4', name: 'Cooking', required: false, multiSelect: false, options: [{ id: 'mg4-rare', name: 'Rare', price: 0 }, { id: 'mg4-mr', name: 'Medium Rare', price: 0 }, { id: 'mg4-med', name: 'Medium', price: 0 }, { id: 'mg4-mw', name: 'Medium Well', price: 0 }, { id: 'mg4-well', name: 'Well Done', price: 0 }] },
            { id: 'mg5', name: 'Drink Size', required: true, multiSelect: false, options: [{ id: 'mg5-s', name: 'Regular', price: 0 }, { id: 'mg5-l', name: 'Large', price: 150 }] },
        ],
        menuItems: [
...
function loadState() {
    try {
        if (fs.existsSync(DB_FILE)) {
            ...
            console.log('[DB] Loaded state from db.json');
            return;
        }
    } catch (err) {
...
// Helper: set a deep path in state
...
// Helper: get a deep path from state
...
// Broadcast helper for clean communication
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (msg.type === 'SET') {
                ...
                saveState();
                broadcast({ type: 'STATE_CHANGE', path: msg.path, value: msg.value });
            }
            else if (msg.type === 'CREATE_ORDER') {
                const order = msg.order;
                order.orderNumber = state.nextOrderNumber || 101;
                state.nextOrderNumber = (state.nextOrderNumber || 101) + 1;
                order.createdAt = new Date().toISOString();
                state.orders.push(order);
                saveState();
                broadcast({ type: 'STATE_CHANGE', path: 'orders', value: state.orders });
                broadcast({ type: 'STATE_CHANGE', path: 'nextOrderNumber', value: state.nextOrderNumber });
            }
            else if (msg.type === 'EVENT') {
                if (msg.event === 'order:dismissed') {
                    const payload = msg.payload || {};
                    const targetId = payload.orderNumber || payload;
                    console.log(`[WS] Attempting dismissal of: ${targetId}`);

                    const order = state.orders.find(o =>
                        (o.orderNumber && String(o.orderNumber) === String(targetId)) ||
                        (o.id === String(targetId))
                    );
                    ...
                    if (order) {
                        ...
                        saveState();
                        broadcast({ type: 'STATE_CHANGE', path: 'orders', value: state.orders });
                        console.log(`[WS] Order ${targetId} dismissed successfully`);
                    } else {
                        console.warn(`[WS] Order ${targetId} NOT found in state`);
                    }
                }
                // Relay custom events to others
...
console.log(`[DB] Server running on ws://0.0.0.0:${PORT}`);
console.log(`[DB] State file: ${DB_FILE}`);
